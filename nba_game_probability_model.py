...
def scrape_nba_data():
    ...
    for month in season_months:
        ...
        if not url_table:
            ...
            continue  # Skip this month if no table is found

        for url_row in url_table.find_all('tr')[1:]:
            # Extract the row data
            row = [url_row.find('th').text] if url_row.find('th') else []  # Handle if 'th' is missing
            row.extend([i.text for i in url_row.find_all('td')])

            # Ensure the row has the right number of elements
            if len(row) < len(headers):
                row.extend([None] * (len(headers) - len(row)))  # Pad with None if there are missing columns
            elif len(row) > len(headers):
                row = row[:len(headers)]  # Truncate if there are extra columns
