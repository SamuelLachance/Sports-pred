...
class Team():
    ...
    def calc_win_pct(self):
        ...
        if total_games == 0:
            return 0  # or you could return NaN if you prefer
        win_percentage = self.wins / total_games
...
def fetch_odds_data(date, predict):
    base_url = f"https://www.oddsshark.com/api/scores/ncaab/{date}?_format=json"
    print(base_url)
    headers = {
...
team_mapping = {
    ...
    "BYU": "BYU Cougars",
    "UConn": "UConn Huskies",  # Note: UConn is often used for University of Connecticut Huskies
    ...
    "SMU": "SMU Mustangs",  # Note: SMU often refers to Southern Methodist University Mustangs
    ...
    "UC-San Diego": "UC San Diego Tritons",  # Note: UC San Diego is not in the ncaa_d1_teams list, assuming it's for future or updated list
    "Utah": "Utah Utes",
    ...
    "Georgetown": "Georgetown University Hoyas",
    ...
    "Loyola (IL)": "Loyola-Chicago Ramblers",
    ...
    "Southern Illinois": "Southern Illinois-Edwardsville Cougars",
    ...
    "St. Francis (PA": "Saint Francis (PA) Red Flash",  # Note: Assuming missing closing parenthesis
...
def get_closest_match(team_name, choices, threshold=40):
...
def merge_odds_and_projections(odds_df, power_df):
    ...
    # Create a dictionary for quick lookup of team data from power_df
    power_dict = {team: data for team, data in power_df.set_index('Team').to_dict('index').items()}
    
    # Create a new DataFrame to hold the merged results
    merged_rows = []

    for index, row in odds_df.iterrows():
        # Map the team names from odds_df to names without mascots if available
        home_team_mapped = next((k for k, v in team_mapping.items() if v == row['Home Name']), row['Home Name'])
        print(home_team_mapped)
        away_team_mapped = next((k for k, v in team_mapping.items() if v == row['Away Name']), row['Away Name'])
        print(away_team_mapped)
        
        # Use the mapped names (without mascots) to get the corresponding data from power_df
        home_team_data = power_dict.get(home_team_mapped)
        away_team_data = power_dict.get(away_team_mapped)
        ...
        if home_team_data and away_team_data:
            ...
            print(f"Could not match teams: {row['Home Name']} or {row['Away Name']}")
...
##        game_row['Win by 21+ Points (Away)'] = f'{calc_spread_2(away_power, home_power, param, 20.5, "inf") * 100:.2f}%'
    ...
    final_game_probability_df.dropna(inplace=True)
